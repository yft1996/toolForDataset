...
int main( int argc, char** argv )
{
    ...
    std::string method="ICL";
    std::string data_path="/home/anshuai/Download/";
    boost::format pose_path("%s/%s/%s_pose.txt");
    ...
    {
        boost::format fmt( "/home/anshuai/Download/%s/%s/%d.%s" ); //图像文件格式
        colorImgs.push_back( cv::imread( (fmt%method%"reference"%(i+1)%"png").str() ));
        depthImgs.push_back( cv::imread( (fmt%method%"depth"%(i+1)%"png").str(), -1 )); // 使用-1读取原始图像
