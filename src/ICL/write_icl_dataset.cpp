...
int main()
{
    ...
    string output_file="pose.txt";
    string output_path="/home/anshuai/Download/ICL/ICL/";

    ifstream asso_in(file_path+associate_file);
    ifstream pose_in(file_path+pose_file);

    ofstream output(output_path+output_file);
    ...
    while(pose_in.good())
    {
        ...
        cv::imwrite(output_path+depth_name,depth);
        cv::imwrite(output_path+rgb_name,rgb);
    }
